 ...
 */
public class QEDDialect extends SqlDialect
{
	/* (non-Javadoc)
		...
		for (Iterator colIt = table.getColumns().iterator(); colIt.hasNext();)
		{
			...
			switch (col.getType())
			{
				...
				case ColumnDefinition.TYPE_BINARY:
					...
					if (size == 0)
						size = 64 * 1024;
					buffer.append("VARBINARY(").append(size).append(")");
					break;
			}
	...
	private class QedSqlGenerator extends SqlGenerator
	{
...
//				sql.append("TOP " + query.getTop() + " ");
			...
			if (query.getSortings() != null)
			{
				...
				{
					SelectQuery.Sorting sort = (SelectQuery.Sorting) sortingIt.next();
