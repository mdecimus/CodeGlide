...
public class MySqlDialect extends SqlDialect
{
	...
	public String[] generateTableStatements(TableDefinition table)
	{
		...
		for (Iterator colIt = table.getColumns().iterator(); colIt.hasNext();)
		{
			...
			switch (col.getType())
			{
				...
				case ColumnDefinition.TYPE_BINARY:
					...
					else if (col.getSize() < (1 << 8))
						buffer.append("TINYBLOB");
					else if (col.getSize() < (1 << 16))
						buffer.append("BLOB");
					else if (col.getSize() < (1 << 24))
						buffer.append("MEDIUMBLOB");
					else
						buffer.append("LONGBLOB");
					break;
			}
			...
			if (col.isPrimaryKey())
			{
				if (col.getType() == ColumnDefinition.TYPE_VARCHAR && col.getSize() > 255)
				{
					statements.add("CREATE UNIQUE INDEX " + table.getName() + "_PK ON " + table.getName() + "(" + col.getName() + "(255))");
		...
		buffer.append(") TYPE=InnoDB AUTO_INCREMENT=1");
		...
		for (Iterator indexIt = table.getIndexes().iterator(); indexIt.hasNext();)
		{
			...
			for (int i = 0; i < columns.length; i++)
			{
				...
				if (column != null && ((column.getType() == ColumnDefinition.TYPE_TEXT) || (column.getType() == ColumnDefinition.TYPE_VARCHAR && column.getSize() > 255)) )
					buffer.append('(').append(255).append(')');
	...
	private class MySqlGenerator extends SqlGenerator
	{
		...
		public void visit(SelectQuery query)
		{
			...
			if (query.getTop() > 0) {
				...
				if( query.getOffset() > -1)
					sql.append(String.valueOf(query.getOffset())).append(",");
				sql.append(String.valueOf(query.getTop()));
			}
		}
